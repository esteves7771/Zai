  ...
  return (
    <nav style={{ height: 'var(--nav-height)', paddingBottom: 'var(--safe-bottom)', background: 'var(--surface)', borderTop: '1px solid var(--border)', display: 'flex', alignItems: 'center', justifyContent: 'space-around', flexShrink: 0, position: 'relative', zIndex: 50 }}>
      {TABS.map(tab => {
        ...
        return (
          <button key={tab.id} onClick={() => onChange(tab.id)} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 4, flex: 1, height: '100%', color: isActive ? 'var(--green-dark)' : 'var(--text-light)', transition: 'color 0.2s ease', position: 'relative' }}>
            {tab.center ? (
              ...
              <>
                <Icon size={22} color={isActive ? 'var(--green-dark)' : 'var(--text-light)'} />
                <span style={{ fontSize: 10, fontFamily: 'Nunito, sans-serif', fontWeight: isActive ? 700 : 400, letterSpacing: '0.02em' }}>{label}</span>
              </>
            )}
          </button>
        )
