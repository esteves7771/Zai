...
export default function ScoreRing({ score, size = 96, lang = 'en' }) {
  ...
  const label = T.score?.[labelKey] || labelKey
  const r = (size / 2) - 6
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 6, position: 'relative' }}>
      <button onClick={() => setShowTooltip(true)} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 4, background: 'none', border: 'none', cursor: 'pointer', padding: 0 }}>
        ...
        <span style={{ fontSize: 10, color: 'var(--text-light)' }}>ⓘ {T.score?.tapForDetails || 'tap for details'}</span>
      </button>
      ...
      {showTooltip && (
        <div style={{ position: 'fixed', inset: 0, zIndex: 300, background: 'rgba(0,0,0,0.4)', display: 'flex', alignItems: 'flex-end' }} onClick={() => setShowTooltip(false)}>
          <div style={{ background: 'var(--surface)', borderRadius: '24px 24px 0 0', width: '100%', padding: '28px 24px 48px', animation: 'slideUp 0.3s ease' }} onClick={e => e.stopPropagation()}>
            <h3 style={{ fontFamily: 'Nunito, sans-serif', fontWeight: 800, fontSize: 18, marginBottom: 20 }}>{T.score?.tooltip?.title || 'How is this score calculated?'}</h3>
            {[
